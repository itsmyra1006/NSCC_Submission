        ...
        <header className="bg-white/95 backdrop-blur-sm shadow-sm sticky top-0 z-50">
            <div className="container mx-auto px-4 sm:px-6 lg:px-8">
                <div className="flex justify-between items-center py-4">
                    <div 
                        className="text-2xl font-bold text-slate-800 cursor-pointer hover:text-purple-600 transition-colors"
                        onClick={() => navigate('/')}
                    >
                        Zenith
                    </div>
                    <div className="flex items-center space-x-4">
                        {user ? (
                            <>
                                <button
                                    onClick={() => navigate('/create-post')} // *** THIS IS THE FIX ***
                                ...
                                <div className="relative" ref={dropdownRef}>
                                    ...
                                    {dropdownOpen && (
                                        <div className="origin-top-right absolute right-0 mt-2 w-48 rounded-md shadow-lg py-1 bg-white ring-1 ring-black ring-opacity-5 z-50">
                                            ...
                                            <a href="#" onClick={(e) => { e.preventDefault(); navigate('/dashboard'); setDropdownOpen(false); }} className="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">Dashboard</a>
                                            <a href="#" onClick={(e) => { e.preventDefault(); navigate('/create-post'); setDropdownOpen(false); }} className="block sm:hidden px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">Create Post</a>
                                            <a href="#" onClick={(e) => { e.preventDefault(); logout(); setDropdownOpen(false); }} className="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">Logout</a>
                                        </div>
                                    )}
                                </div>
                            </>
